...
/**
 * Clase de Prueba de Integración para GestorReservas.
 * * Utiliza @SpringBootTest para cargar el contexto completo de la aplicación,
 * incluyendo la configuración real de JPA/Hibernate y la BD H2 en memoria.
 * * @Transactional asegura que la BD se revierta (limpie) después de cada test.
 */
@SpringBootTest
@Transactional
public class GestorReservasTest {
    ...
    public void testCrearReserva_PersistenciaExitosa() {
        ...
        // Aseguramos que la BD esté limpia de registros con este apellido al inicio (aunque @Transactional ayuda)
        reservaRepo.findByApellido("Gomez").forEach(reservaRepo::delete);

        // --- ACT (Ejecución de la lógica completa) ---
        // Esto llama al GestorReservas, que utiliza el ReservaRepository REAL para guardar en H2
        String resultadoOperacion = gestorReservas.crearReserva(
            huesped, tipoStr, numeroHab, fechaInicioStr, fechaFinStr);

        // --- ASSERT (Verificación) ---
        
        // 1. Verificar la respuesta del Gestor
        assertEquals("¡Reserva Exitosa!", resultadoOperacion, 
                     "El gestor debería retornar éxito.");

        // 2. Verificar la Persistencia REAL en H2 (La clave de la prueba de integración)
        ...
        // Opcional: Verificar un detalle del objeto guardado
        Reserva reservaGuardada = reservas.get(0);
        assertEquals("Laura", reservaGuardada.getNombre(), 
                     "El nombre guardado en la BD no coincide.");
        
    }
    
    
    /**
     * Prueba de Integración: Verifica que no se guarda nada si la habitación está ocupada.
     */
    @Test
    public void testCrearReserva_PersistenciaFallaPorOcupacion() throws Exception {
        // --- ARRANGE (Preparación de datos) ---
        // Paso 1: Crear una reserva inicial que OCUPE la habitación (Esto se guarda en la BD)
        HuespedDTO huesped1 = new HuespedDTO(new Huesped("Ocupante", "Inicial", TipoDoc.DNI,
           "11111111", null,"Pais","inicial@test.com","123","Mama",false));
        gestorReservas.crearReserva(huesped1, "Individual estándar", 101, "01/01/2026", "05/01/2026");
        
        // Paso 2: Intentar crear una reserva que colisiona en fechas
        HuespedDTO huesped2 = new HuespedDTO(new Huesped("Colision", "Fallido", TipoDoc.DNI,
